...
KERNEL_MODULES_INSTALL_STAMP := $(KERNEL_OUT)/.modules_stamp
...
KERNEL_MODULES_INTERMEDIATES := $(call intermediates-dir-for,PACKAGING,kernel_modules)
...
# Install it
$(PRODUCT_OUT)/kernel: $(TARGET_PREBUILT_INT_KERNEL) $(TARGET_KERNEL_MODULES) | $(ACP)
...
$(KERNEL_MODULES_INSTALL_STAMP): $(KERNEL_OUT_STAMP) $(KERNEL_CONFIG)
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			echo "Building Kernel Modules"; \
			$(call make-kernel-target,modules) || exit "$$?"; \
			echo "Installing Kernel Modules"; \
			$(call make-kernel-target,INSTALL_MOD_PATH=$(KERNEL_MODULES_INTERMEDIATES) INSTALL_MOD_STRIP=1 modules_install); \
			kernel_release=$$(cat $(KERNEL_RELEASE)) \
			modules=$$(find $(KERNEL_MODULES_INTERMEDIATES)/lib/modules/$$kernel_release -type f -name '*.ko'); \
			($(call build-image-kernel-modules,$$modules,$(KERNEL_MODULES_OUT),$(KERNEL_MODULE_MOUNTPOINT)/,$(KERNEL_DEPMOD_STAGING_DIR))); \
			fi
	$(hide) touch $@

TARGET_KERNEL_MODULES := $(KERNEL_MODULES_INSTALL_STAMP)

.PHONY: kernelmodules
kernelmodules: $(TARGET_KERNEL_MODULES)
